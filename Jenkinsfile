  ...
  stages {
    ...
    stage('Run Acceptance Tests') {
      steps {
        ...
        def runscript='/opt/rh/rh-maven33/root/usr/bin/mvn clean -Dtest=gov.dhs.nppd.humanreview.SerenityTest -Dhuman.review.rest.url=http://human-review-backend-labs-test.apps.domino.rht-labs.com/api/v1 -Dhr.regular.username=User1 -Dhr.regular.password=Pass1 clean test'
        def retVal = sh(returnStatus: true, script: runscript)
